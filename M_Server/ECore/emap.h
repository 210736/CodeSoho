...
private:
	...
	INLINE void rb_destroy(tagNode *x)
	{
		if (x!=&rb_null)
		{
			if (x->left!=&rb_null)
	...
	INLINE tagNode* rb_insert(KEY_TYPE key, VALUE_TYPE value)
	{
	...
	INLINE tagNode* rb_lookup(KEY_TYPE key)
	{
		tagNode *x=rb_root;
	...
	INLINE void rb_left_rotate(tagNode *x)
	{
		tagNode *t = x->right;
		x->right = t->left;
	...
	INLINE void rb_right_rotate(tagNode *y)
	{
		tagNode *t = y->left;
		y->left = t->right;
	...
	INLINE tagNode* rb_successor(const tagNode *x) const
	{
		tagNode* t;
		if (x->right!=&rb_null)
	...
	INLINE tagNode* rb_predecessor(const tagNode *x)
	{
		tagNode* t;
		if( x->left!=&rb_null )
	...
	INLINE void rb_delete(tagNode *z)
	{
		tagNode *x, *y;
