...
{
   if( m_nError == 0 )
      _stprintf_s(lpstrError, nMaxError, _T("%s error"), m_strMessage.c_str());
   else
      _stprintf_s(lpstrError, nMaxError, _T("%s error 0x%08x"), m_strMessage.c_str(), m_nError);
   return TRUE;
}
...
int CHttpBlockingSocket::ReadHttpResponse(char* pch, int nSize, int nSecs)
{
   ...
   { // now pass the rest of the data directly to the caller
      nBytesToRead = EMin(static_cast<int>(nSizeRecv), nSize - nBytesRead);
