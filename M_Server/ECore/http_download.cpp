...
BOOL HttpDownload::Connect(const char *szHostName, int nPort)
{
	...
	m_pStream->Init(TRUE, Lan::ECM_Text);
...
// 根据请求的相对URL输出HTTP请求头
//-----------------------------------------------------------------------------
size_t HttpDownload::FormatRequestHeader(const char *pServer,const char *pObject, INT64 nFrom)
{
	ZeroMemory(m_szRequestHeader, sizeof(m_szRequestHeader));
...
///发送请求头
//-----------------------------------------------------------------------------
BOOL HttpDownload::SendRequest(const char *pRequestHeader, size_t Length)
{
...
BOOL HttpDownload::Download(LPCTSTR szSrcFileName, BOOL bCache)
{
	...
	// 启动连接线程
	CallbackBase* pExec	= new CallbackP0<HttpDownload>(this, &HttpDownload::DownloadThread);
	CallbackBase* pEnd	= new CallbackP0<HttpDownload>(this, &HttpDownload::Terminate);
...
// 下载线程
//-----------------------------------------------------------------------------
DWORD HttpDownload::DownloadThread()
{
	DWORD dwServiceType;
	INTERNET_PORT nPort;
	tstring strServer,strObject;
	std::vector<tstring> vecFileNames;
	StringUtil::StringToToken(vecFileNames, m_strSrcFileName.c_str(), _T(','));
...
_download_thread_start:
	...
	std::string strServerA = SS::ToA(strServer.c_str());
	std::string strObjectA = SS::ToA(strObject.c_str());
	...
	size_t nLength = this->FormatRequestHeader(strServerA.c_str(),strObjectA.c_str(), m_n64CompletedSize);	
	...
	if( nServerState > 299 && nServerState < 399 )	// 需要重定向
	{
		...
		{
			m_strSrcFileName = SS::ToW(strNewLocation.c_str());
			m_pStream->Disconnect();
