...
PVOID EnumProcess::GetModulePreferredBase(DWORD dwPID, PVOID pModBase)
{
	...
	if (hProc)
	{
		...
		if (IMAGE_NT_SIGNATURE == inh.Signature) // NT signature OK?
			return (PVOID)(SIZE_T)inh.OptionalHeader.ImageBase;	// Get the preferred base...
	}
...
bool	Process::ExecuteProcess( const TCHAR* szProcessName )
{
...
bool	Process::QuitProcess( const TCHAR* szProcessName, BOOL bFullPath /* = FALSE  */)
{
...
bool	Process::ShutDownProcess( const TCHAR* szProcessName, BOOL bFullPath /* = FALSE  */)
{
	HANDLE ProcessHandle[1024];
	DWORD dwProcessID[1024];
	ZeroMemory(ProcessHandle, sizeof(HANDLE)*1024);
...
INT Process::GetProcNum( DWORD& dwProcessID, const TCHAR* szProcessName, BOOL bFullPath /* = FALSE  */ )
{
	...
	for( BOOL bRet = enumeration.GetProcessFirst( &entry ); bRet; bRet = enumeration.GetProcessNext( &entry ) )
	{
		...
		{
			//包含
			if( TRUE == _tcsstr_tail(entry.lpFilename, szProcessName) )
			{
				nNum++;
			}
		}
...
HANDLE Process::FindProcess( DWORD& dwProcessID, const TCHAR* strProcessName, BOOL bFullPath /* = FALSE  */)
{
...
int		Process::FindAllProcess( HANDLE* pProcessHandle, DWORD* dwID, const TCHAR* strProcessName, BOOL bFullPath /* = FALSE  */)
{
	HANDLE hProcess;
	INT nProcessNum = 0;
	...
	for ( BOOL bRet = enumeration.GetProcessFirst( &entry ); bRet; bRet = enumeration.GetProcessNext( &entry ) )
	{
		...
		
		{
			...
			{
				if( TRUE == _tcsstr_tail(entry.lpFilename, strProcessName) )
				{
					dwID[nProcessNum] = entry.dwPID ;
					pProcessHandle[nProcessNum++] = hProcess;
				}
...
void Process::TerminateMe()
{
	HANDLE Killprcshd = ::OpenProcess(PROCESS_TERMINATE, FALSE, ::GetCurrentProcessId() );
	if( Killprcshd )
	{
		if( ::TerminateProcess(Killprcshd, 0) )
		{
			::WaitForSingleObject(Killprcshd, 5000);
		}
		::CloseHandle(Killprcshd);
	}
